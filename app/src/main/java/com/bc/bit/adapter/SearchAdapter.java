...
public class SearchAdapter extends BaseQuickAdapter<TalkBean, BaseViewHolder> implements LoadMoreModule {
    ...
    protected void convert(@NotNull BaseViewHolder helper, @NotNull TalkBean item) {
        TextView item_search_name =  helper.getView(R.id.item_search_name);
        TextView item_search_time =  helper.getView(R.id.item_search_time);
        TextView item_search_content =  helper.getView(R.id.item_search_content);
        RoundedImageView item_search_image_header =  helper.getView(R.id.item_search_image_header);
        //获取列表控件
        RecyclerView rv = helper.getView(R.id.rv);

        item_search_name.setText(item.getUser().getNickName());
        item_search_time.setText(TimeUtil.commonFormat(item.getPublishTime()- Constant.TIME_DIFF));
        item_search_content.setText(item.getContent());

        Glide.with(getContext()).load(item.getUser().getHead())
                .apply(new RequestOptions().placeholder(R.drawable.pic_me_placeholder))
                .error(R.drawable.pic_me_placeholder).into(item_search_image_header);
        ...
        if (picPaths != null && picPaths.size() > 0) {
            ...
            adapter.setOnItemClickListener(new OnItemClickListener() {
                ...
                public void onItemClick(BaseQuickAdapter adapter, View view, int position) {
                    ...
                    //回调监听器
                    searchListener.onImageClick(rv, picPaths, position);
                }
            });
