            ...
            public void onGlobalLayout() {
                if (isExpand) {
                    ...
                    if (ellipsizeStr.length() < originText.length()) {
                        ...
                        ssb.setSpan(new ForegroundColorSpan(Color.parseColor("#C0E0A1")),
                                temp.length() - endText.length(), temp.length(),
                                Spannable.SPAN_INCLUSIVE_EXCLUSIVE);
                        ssb.setSpan(new AbsoluteSizeSpan(37), temp.length() - endText.length(), temp.length(), Spannable.SPAN_INCLUSIVE_INCLUSIVE);
