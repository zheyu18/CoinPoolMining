...
import com.scrb.lib.entity.ICandle;
...
public class MainDraw implements IChartDraw<ICandle> {
    ...
    private int m5Color;
    private int m10Color;
    private int m30Color;
    ...
    private void drawSelector(BaseKLineChartView view, Canvas canvas) {
        ...
        strings.add("   时间 "+spacing+view.getAdapter().getDate(index));
        strings.add("最高价"+spacing + point.getHighPrice());
        strings.add("最低价"+spacing+ point.getLowPrice());
        strings.add("开盘价"+spacing+point.getOpenPrice());
        strings.add("收盘价"+spacing + point.getClosePrice());
        strings.add("交易量"+spacing + point.getVol());
        ...
        //绘制边框
        RectF rectFBorder = new RectF(left, top, left + width, top + height);
        mSelectorBackgroundPaint.setStyle(Paint.Style.STROKE);
        mSelectorBackgroundPaint.setStrokeWidth(1);
        mSelectorBackgroundPaint.setColor(mSelectorTextPaint.getColor());
        canvas.drawRect(rectFBorder, mSelectorBackgroundPaint);
        //绘制实体
        mSelectorBackgroundPaint.setColor(mContext.getResources().getColor(R.color.k_line_select_dialog_bg_color));
        mSelectorBackgroundPaint.setStyle(Paint.Style.FILL);
        int tagSize = ViewUtil.Dp2Px(mContext,1);
        RectF rectFCenter = new RectF(left+tagSize, top+tagSize, (left + width)- tagSize, (top + height)- tagSize);
        canvas.drawRect(rectFCenter, mSelectorBackgroundPaint);
