...
import com.scrb.lib.draw.KDJDraw;
import com.scrb.lib.draw.MACDDraw;
import com.scrb.lib.draw.MainDraw;
import com.scrb.lib.draw.RSIDraw;
import com.scrb.lib.draw.VolumeDraw;
import com.scrb.lib.draw.WRDraw;
...
public class KLineChartView extends BaseKLineChartView {
    ...
    private void initEmptyView() {
        mEmpyView = LayoutInflater.from(mContext).inflate(R.layout.k_line_chart_empty, null);
        LayoutParams layoutParams = new LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.MATCH_PARENT);
        mEmptyRefreshBtn = mEmpyView.findViewById(R.id.k_line_chart_empty_refresh_btn);
        mEmptyRefreshBtn.setOnClickListener(new OnClickListener() {
            @Override
        ...
        mEmptyImageView = mEmpyView.findViewById(R.id.k_line_chart_empty_img);
        mEmptyTextView = mEmpyView.findViewById(R.id.k_line_chart_empty_text);
        addView(mEmpyView, layoutParams);
        mEmpyView.setVisibility(GONE);
    }
    ...
    public void justShowLoading() {
        if (!isRefreshing) {
            ...
            if (mEmpyView.getVisibility() == VISIBLE) {
                mEmpyView.setVisibility(GONE);
            }
            if (mRefreshListener != null) {
                mRefreshListener.onLoadMoreBegin(this);
     ...
     * 显示占位图
     */
    public void showNoEmptyView() {
        if (mEmptyRefreshBtn!=null){
            mEmptyRefreshBtn.setVisibility(GONE);
        }
        if (mEmpyView != null) {
            mEmpyView.setVisibility(VISIBLE);
        }
        if (mEmptyImageView!=null){
            mEmptyImageView.setImageDrawable(mContext.getResources().getDrawable(R.drawable.ic_empty));
        }
        if (mEmptyTextView!=null){
            mEmptyTextView.setText(mContext.getResources().getString(R.string.no_data));
        }
    }
    ...
    public void showNetErrorView() {
        ...
        if (mEmptyImageView!=null){
            mEmptyImageView.setImageDrawable(mContext.getResources().getDrawable(R.drawable.ic_net_work_error));
        }
        if (mEmptyTextView!=null){
            mEmptyTextView.setText(mContext.getResources().getString(R.string.net_work_error));
        }
        if (mEmptyRefreshBtn!=null){
            mEmptyRefreshBtn.setVisibility(VISIBLE);
        }
    }
